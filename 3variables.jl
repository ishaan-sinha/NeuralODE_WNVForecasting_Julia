...
df = DataFrame(A=Float32[], B=Float32[], C=Float32[])
...
for year in range(2015, 2015)
    ...
    function diffEqError(x)
        ...
        u0 = [499*x[1], x[1], x[2], 0, 39000000, 0, 0, 0, 0]
        p = [x[3]]
        t_end = 181
        tspan = (0.0, t_end)
    ...
    u0 = [499*result.minimizer[1], result.minimizer[1], 0, result.minimizer[2], 39000000, 0, 0, 0, 0]
    p = [result.minimizer[3]]
    t_end = 181
    tspan = (0.0, t_end)
    prob = ODEProblem(wnv, u0, tspan, p)
    sol = solve(prob, Rodas5(), saveat=1, dt=1e-4)
    ...
    savefig("3variables_CA_" * string(year)*".png")
    push!(df, (result.minimizer[1], result.minimizer[2], result.minimizer[3]))
end
