...
for state in ["CA"]
    ...
    for year in range(2000 , 2022)
        ...
        ubounds = [2000000.0, 100000.0, 10000000.0, 200000.0, 200000.0]

        #result = optimize(diffEqError, lbounds, ubounds, x)
        #result = optimize(diffEqError, lbounds, ubounds, x, Fminbox(GradientDescent()))

        #result = optimize(diffEqError, lbounds, ubounds, x, Fminbox(GradientDescent()), Optim.Options(time_limit=60.0); autodiff=:forward)
        #result = optimize(diffEqError, lbounds, ubounds, x, Fminbox(ConjugateGradient()), Optim.Options(time_limit=60.0); autodiff=:forward)
    ...
    CSV.write("LineSearch5variables_"*state*".csv", df)
end